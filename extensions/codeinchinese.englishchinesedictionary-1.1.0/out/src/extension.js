...
function activate(context) {
    ...
    let 启用悬停 = vscode_1.workspace.getConfiguration().get(启用悬停配置);
    context.subscriptions.push(vscode_1.workspace.onDidChangeConfiguration((事件) => {
        if (事件.affectsConfiguration(启用悬停配置)) {
            启用悬停 = vscode_1.workspace.getConfiguration().get(启用悬停配置);
        }
    }));
    // 注册hover提供器
    context.subscriptions.push(vscode_1.languages.registerHoverProvider({
        pattern: '**',
    }, {
        provideHover: (文档, 位置) => {
            if (启用悬停) {
                ...
                return new vscode_1.Hover(翻译结果);
            }
        }
    }));
}
